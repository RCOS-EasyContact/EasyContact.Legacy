 ...
 public:
    ...
    static void register_router(HttpService* router) {
        ...
        router->GET("/tags/:tagName", [](HttpRequest* req, HttpResponse* resp) {
            ...
            if(tagName == ""){ return 400; }
            std::unordered_set<std::string> contacts = 
                getTagContains(tagName);
            if(contacts.empty()){ return 404; }
        ...
        router->POST("/tags/:tagName", 
            [](HttpRequest* req, HttpResponse* resp) {
            std::string tagName = req->GetParam("tagName");
            if(tagName == ""){ return 400; }
            if(newTag(tagName)){ return 200; }
            return 406;
        });        
        ...
        router->Delete("/tags/:tagName", [](HttpRequest* req, HttpResponse* resp) {
            std::string tagName = req->GetParam("tagName");
            if(tagName == ""){ return 400; }
            if(removeTag(tagName)){ return 200; }
            return 404;
        });
